...
if($seasonid=="")
	...
	if($forumid=="")
		...
		{
		if($_POST['p_text']==""||$_POST['p_caption']=="")
			{
			if(($threadmodus!=2&&$threadmodus!=3)||$headeruserrang==2)
				...
				echo "Text:<br><textarea name=\"p_text\" rows=\"14\" cols=\"60\"></textarea><br>";
			...
			$caption=htmlentities($_POST['p_caption']);
			$text=htmlentities($_POST['p_text']);
			$thistime=time();
			...
			if((strtotime($lastpost)+60)>$thistime)
				...
				echo "\n<br><a href=\"index.php?mode=threads&amp;forumid=$forumid&amp;season=$seasonid\">Zum Forum zur&uuml;ck</a><br>";
				}
			else
				{
				if(($threadmodus!=2&&$threadmodus!=3)||$headeruserrang==2)
